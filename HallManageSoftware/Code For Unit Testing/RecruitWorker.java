...
public class RecruitWorker extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 6691143666188860443L;
	...
	public static void main(String[] args) {
		try {
			RecruitWorker dialog = new RecruitWorker("VS");
	 ...
	 * Create the frame.
	 */
	public RecruitWorker(String hallcode) {
/********************Extra Code************************/
		...
		recruit = new JButton("Recruit Worker");
	...
	private void searchWorker() throws SQLException {
		...
		String query = "SELECT * FROM `workerlist` WHERE `type` LIKE '"
				+ workerType + "' AND `hallcode` LIKE 'not recruited'";
...
/*		query = "UPDATE `hms`.`workerlist` SET `hallcode` = '" + hallCode
				+ "' WHERE `workerlist`.`no` =" + key[sindex - 1];
*/
		
		/*******************Code For Unit Testing*****************/
		
		query = "UPDATE `hms`.`workerlist` SET `hallcode` = '" + hallCode
				+ "' WHERE `workerlist`.`no` =" + 7;
